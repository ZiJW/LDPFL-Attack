...
class PrivFL_server(Base_server):
    ...
    def train(self, rn):
        ...
        if param.TAPPING_CLIENTS == [] :
            Thr = [threading.Thread(target=PrivFL_server.collect_weights, args=(self, idx)) for idx in chose]
            for tr in Thr:
                tr.start()
            for tr in Thr:
                tr.join()
            ...
            Thr = [threading.Thread(target=PrivFL_server.collect_weights, args=(self, idx)) for idx in chose if idx not in param.TAPPING_CLIENTS]
            ...
            Thr = [threading.Thread(target=PrivFL_server.collect_weights, args=(self, idx)) for idx in chose if idx in param.TAPPING_CLIENTS]
