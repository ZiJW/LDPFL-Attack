...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes, in_channel):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        if stride != 1 or in_planes != self.expansion * planes:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, self.expansion * planes,
                          kernel_size=1, stride=stride, bias=False),
                nn.GroupNorm(num_groups=8, num_channels=self.expansion * planes)
            )
...
class Bottleneck_GN(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.conv3 = nn.Conv2d(planes, self.expansion *
                               planes, kernel_size=1, bias=False)
        self.gn3 = nn.GroupNorm(num_groups=8, num_channels=self.expansion * planes)
...
def ResNet18_GN():
    return ResNet_GN(BasicBlock_GN, [2, 2, 2, 2], 10)
