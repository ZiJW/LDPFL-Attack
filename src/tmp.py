...
class DPSGD_client(Base_client):  
    ...
    def __init__(self, id):
        ...
        if self.ldp and False:
            self.privacyEngine = PrivacyEngine()
            self.model, self.optimizer, self.train_loader = self.privacyEngine.make_private(
                module=self.model,
                optimizer=self.optimizer,
                data_loader=self.train_loader,
                noise_multiplier=param.SIGMA,
                max_grad_norm=param.NORM_BOUND,
            )
